...
def loadglovemodelwiki(File, num = 400):
    glove_model = {}
    j = 0
    p = 0
    q = 0
    ...
    with open(File,'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            word = split_line[0]
            try:
                # Convert the rest of the elements to floats (embedding)
                embedding = np.array(split_line[1:], dtype=np.float64)
                if len(embedding) != num:
                    print(f"Number is wrong {i + 1}: {split_line[1]}")
                    print("------------")
                    p = p+1
                else:
                    if word in glove_model:
                        print(f"The word is already in the GloVe {i + 1}: '{word}'")
                        print("------------")
                        q = q+1
                    else:
                        glove_model[word] = embedding
                ...
                print(f"Error in line {i + 1}:{line[0:10]} '{word}' due to: {e}")
                print("------------")
                d = d+1
    return glove_model, j, p, q, d, k
    ...
    with open(File,'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            try:
                ...
                if len(split_line) == 1:
                    print(f"Number is wrong {i + 1}: {split_line}")
                    print("------------")
                    p = p+1
                else:
                    if word in glove_model_vocab:
                        print(f"The word is already in the GloVe {i + 1}: '{word}'")
                        print("------------")
                        q = q+1
                    else:
                ...
                print(f"Error in line {i + 1}: {line} '{word}' due to: {e}")
                print("------------")
                d = d+1
    return glove_model_vocab, j, p, q, d, k
